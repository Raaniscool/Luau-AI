    ...
    checks = (
        (
            (
                ...
                r"\b(?:the\s+)?server\s+(?:must|has\s+to|should|can|may)\s+(?:explicitly\s+)?(?:fire|call|use|invoke)\s+(?:a\s+)?(?:remoteevent\s*:\s*)?fireserver\b",
                r"\bremoteevents?\s+(?:only|just)\s+(?:let|allow)\s+(?:the\s+)?server\s+(?:trigger|initiate|start)\b",
                r"\bremoteevents?\s+(?:are|is)\s+(?:server[\s-]initiated|server[\s-]only)\b",
            ),
        ...
        (
            (
                ...
                r"\bremoteevents?\s+(?:are|is)\s+(?:a|an)\s+(?:secure|safe)\b",
                r"\bremoteevents?\s+(?:are|is)\s+(?:secure|safe)\s+by\s+design\b",
                r"\bremoteevents?\s+(?:are|is)\s+designed\s+to\s+(?:prevent|stop|block)\s+(?:client[\s-]+side\s+)?(?:exploits?|cheating)\b",
                r"\bremoteevents?\s+(?:are|is)\s+(?:more\s+)?(?:secure|safe)\s+than\s+(?:remote)?functions?\b",
